define('app/views/manage/mylikes_icons', [
	'magix',
	...
	l.exports = a.View.extend({
		...
		render: function () {
			var i = this,
				l = i.param('keyword')
			i.wrapModel(e).getFavoritesList(function (c) {
				;(c.favorites = t.filter(c.favorites, function (c) {
					return new RegExp(l).test(c.name)
				})),
					(i.data = { list: c.favorites }),
		...
		'car<click>': function (c) {
			var i = c.params.index,
				i = this.data.list[i],
				c = s(c.target).parents('li')
			c.hasClass('selected')
				? (a.Vframe.get('magix_vf_car').invoke('removeIcon', i),
				  c.removeClass('selected'))
				: (a.Vframe.get('magix_vf_car').invoke('addIcon', i),
				  c.addClass('selected')),
				this.setView()
		},
		'downloadIcon<click>': function (c) {
			c = c.params.id
			t.showDownloadDialog({ iconId: c })
		},
		'cancelFavor<click>': function (c) {
			var i = c.params.id,
				l = c.params.index,
				a = this
			a.wrapModel(n).cancelFavor(i, function (c) {
				a.data.list.splice(l, 1), a.setView()
			})
		},
	})
})
