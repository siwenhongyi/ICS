define('app/views/user/user_collections', [
	'magix',
	'app/exts/helper',
	'jquery',
], function (i, c, l) {
	...
	l.exports = e.View.extend({
		...
		_getType: function () {
			var i = t.parse()
			return 'likes' === i.params.userViewType
				? 'likes'
				: 'collections_illustration' === i.params.userViewType
				? 'illustration'
				: 'icon'
		},
		render: function () {
			var i,
				l = this
			l.rendered && l._beforedata && l.changed('keyword')
				? l._render(l._beforedata)
				: ((i = l.param('uid') || 1),
				  l
						.request()
						.all(
							[
								{
									...
									params: { uid: i, type: this._getType() },
								},
							],
							function (i, c) {
								c = c.get('data')
								;(l._beforedata = c), l._render(c)
							}
						))
		},
		'likeCollection<click>': function (l) {
			var i = l.params.cid,
				e = l.params.index,
				t = this
			t.request().all(
				[{ name: 'collection_like', params: { cid: i } }],
				function (i, c) {
					n(l.currentTarget).addClass('liked'),
						(t.data.collections[e].likes_count += 1),
						t.setView()
				}
			)
		},
