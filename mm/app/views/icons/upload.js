                    ...
                    h.isZh ||
                        ...
                        s.each(a.icons, function(a, e) {
                            if (((e.keepFill = !0), e.aiTagInfo)) {
                                for (
                                    var t = [e.slug], l = h.isZh ? 'name' : 'ename', i = 0; i < v.length; i++
                                ) {
                                    var s = v[i],
                                        o = s[l]
        ...
        isValid: function(a) {
            ...
            if ('collection' === e.advanceType) {
                if ('create' === e.collectionChooseType) {
                    ...
                    if (3 === e.role && a.url)
                        if (!/(http|https):\/\/[\w\-_]+(\.[\w\-_]+)+([\w\-\.,@?^=%&:/~\+#]*[\w\-@?^=%&/~\+#])?/.test(
                                a.url
                            ))
                            return (
                                p.showAlert(
